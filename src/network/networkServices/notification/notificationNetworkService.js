...
async getAllNotification(payload){
    ...
    return await requestService.getRequest(`${GET_ALL_NOTIFICATION}?Id=${id}&userId=${id}&pageNo=${pageNo}&pageSize=3`,
    {
        headers:{
...
async deleteNotification(payload){
    ...
    console.log(id,"id")
    console.log(notificationId,'n_id')
    console.log(token,"token")
    return await requestService.putRequest(`${DELETE_NOTIFICATION}?Id=${id}&notificationId=${notificationId}`,
    {},{
        headers:{
            Authorization: `Bearer ${token}`,
        } 
